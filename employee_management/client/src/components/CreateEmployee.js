...
function CreateEmployee() {
    ...
    let onSubmitValidations=()=>{
        ...
        if(validate.email!=="")
        {
            emailValidate();
            email.map((ele,i)=>{
                if(validate.email===ele.email)
                {
                    return emailSpanRef.current.innerHTML="Email Already Exists";
                }
            })
        }
        else
    ...
    let emails=async()=>{
        ...
        try
        {
            let JSONDataEmail=await fetch("http://localhost:1234/employeesData",reqOptions);
            let JSODataEmail=await JSONDataEmail.json();
            setEmail(JSODataEmail);
            console.log(JSODataEmail);
        }
        catch(err)
        {
            console.log("Unable to fetch employee data(Create Employee)");
            console.log(err);
        }
    };
